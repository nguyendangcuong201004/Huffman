...
bool cmp(pair<char, int> p1, pair<char, int> p2){
    ...
    if (p1.first >= 65 && p1.first <= 90 && p2.first >= 97 && p2.first <= 122){
        return true;
    }
    return false;
}
...
int main(){
    ...
    cout << "Vector v sau khi cong don va sap xep: \n";
    for (auto it : v){
        cout << it.first << ' ' << it.second << endl;
    }
    ...
    for (int i = 0; i < 3; i++){
        Q.push(new HuffTree({v[i].first, Order[i]}, v[i].second, 0));
    }
    cout << "HANG DOI UU TIEN: \n";
    print_queue(Q);
    cout << endl;
    HuffTree* huff_man = BuidHuff(Q);
    cout << endl;
    cout << "DUYET CAY PREORDER: \n";
    huff_man->print(huff_man->getRoot());
    cout << endl;
    end = clock();
    cout << "MAp: \n";
    for (auto it : mp){
    ...
    ans = res.substr(res.size() - 10 ,10);
    reverse(ans.begin(), ans.end());
    cout << ans << endl;
    cout << binaryToDecimal(ans) << endl;
    cout << "Thoi gian chay:  ";
	double tg = 1.0 * (end - start) / CLOCKS_PER_SEC;
	cout << tg << endl;
}